...
export async function softDeleteTx(id: string, userId: string) {
  ...
  try {
    ...
    const row = database.getAllSync(
      `SELECT * FROM transactions WHERE deleted=1 AND synced=0`
    );
    // row.forEach((item) => {
    //   console.log(item);
    // });
...
export async function pageByCursor(
  ...
  let sql = `
    SELECT id, user_id as userId, amount, category, date_iso as dateIso, notes, type, synced, updated_at as updatedAt FROM transactions WHERE user_id=? AND deleted=0
  `;
...
export async function deletedTxPageByCursor(
  ...
  let sql = `
    SELECT id, user_id as userId, amount, category, date_iso as dateIso, notes, type, synced, updated_at as updatedAt FROM transactions WHERE user_id=? AND deleted=1
  `;
  const params: any[] = [userId];
  if (cursor) {
    sql += ` AND (date_iso < ?)`;
